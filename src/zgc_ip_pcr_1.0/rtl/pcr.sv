...
#(
  ...
  logic         pcr_reset_en;
  logic [7:0]   locality;
  logic [255:0] pcr_digest; //pcr计算中产生的中间digest
  logic         pcr_updating;
  ...
  assign pcr_select = reg2hw.pcr_ctrl.select.q;
  assign pcr_extend_en = reg2hw.pcr_ctrl.extend_en.q;
  assign pcr_event_en = reg2hw.pcr_ctrl.event_en.q;
  assign pcr_read_en = reg2hw.pcr_ctrl.rd_en.q;
  assign pcr_reset_en = reg2hw.pcr_ctrl.reset_en.q;
  assign locality = reg2hw.pcr_ctrl.locality.q;
  assign hw2reg.pcr_status.d = !pcr_updating;
  assign hw2reg.pcr_status.de = !pcr_updating;

  always_comb begin
    for (int i = 0; i < 8; i++) begin
      pcr_digest[i*32 +: 32] = hw2reg.digest[i].d;
    end
  end
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      cfg_reg <= '{
        ...
        endian_swap: '{
          q: PCR_CFG_ENDIAN_SWAP_RESVAL,
          qe: 1'b0
        },
        digest_swap: '{
          q: PCR_CFG_DIGEST_SWAP_RESVAL,
          qe: 1'b0
        },
        key_swap: '{
          q: PCR_CFG_KEY_SWAP_RESVAL,
          qe: 1'b0
        },
        digest_size: '{
          q: PCR_CFG_DIGEST_SIZE_RESVAL,
          qe: 1'b0
        },
        key_length: '{
          q: PCR_CFG_KEY_LENGTH_RESVAL,
          qe: 1'b0
        },
        default:'0
      };
  ...
  pcr_core u_hmac (
    ...
    .pcr_extend_en_i(pcr_extend_en),
    .pcr_event_en_i(pcr_event_en),
    .pcr_read_en_i(pcr_read_en),
    .pcr_read_data_o(pcr_read_data),
    .pcr_reset_en_i(pcr_reset_en),
    .locality_i(locality),
    .pcr_hash_done_i(sha_hash_done),
    .digest_i(pcr_digest),
    .pcr_updating_o(pcr_updating)
  );
